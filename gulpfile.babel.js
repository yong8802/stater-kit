...
gulp.task('default', (cb) => {
  runSequence(
    ['sprite'], // svg 아이콘 폰트화 시킬 경우 ,'iconfont' 추가
 ...
 */
gulp.task('copy', ['copy:font', 'copy:sw-scripts', 'copy:jquery', 'copy:animate', 'copy:lib'], () => {
...
gulp.task('copy:jquery', () => {
  gulp.src(config.dir.copy.jquery.src)
    .pipe(gulp.dest(config.dir.copy.jquery.dest));
});
gulp.task('copy:animate', () => {
  gulp.src(config.dir.copy.animate.src)
    .pipe(gulp.dest(config.dir.copy.animate.dest));
});
gulp.task('copy:lib', () => {
  gulp.src(config.dir.copy.lib.src)
    .pipe(gulp.dest(config.dir.copy.lib.dest));
});
gulp.task('copy:sw-scripts', () => {
  gulp.src(['node_modules/sw-toolbox/sw-toolbox.js', 'src/js/sw/runtime-caching.js'])
...
gulp.task('views', () => {
  gulp.src(config.dir.html.src)
    // .pipe($.htmlmin(config.HTML_MIN)) // HTML 압축
